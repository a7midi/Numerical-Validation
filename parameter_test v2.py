# ==============================================================================
# Amended Toy Model for Alayar's Parameter Elimination Theorem
#
# This advanced version incorporates the caveats from the review to create a
# more rigorous and faithful simulation of the theory's principles.
#
# --- AMENDMENTS IMPLEMENTED ---
#
# 1. Deterministic Hidden Layer (Caveat 1):
#    Instead of generating random tags at each tick, the simulation now
#    pre-computes a single, large pseudo-random sequence. The hidden layer
#    draws from this sequence deterministically. This ensures the influx of
#    information is identical across all (q, R) runs, making the comparison
#    of their entropy generation more fair and controlled.
#
# 2. Observer Slice (Caveat 2):
#    Entropy is no longer calculated on all sites. A formal "observer slice"
#    is defined as all nodes EXCEPT the first (hidden) layer. The entropy
#    calculation is restricted to this slice, directly modeling the observer's
#    limited perspective as described in the papers.
#
# 3. Increased Scale (Caveat 3):
#    The number of layers and nodes has been increased to better simulate
#    the dynamics of a larger system, moving closer to the "continuum limit"
#    concept.
#
# 4. Injective Fusion Rule (Caveat 4):
#    The simple hash function has been replaced by a custom class that
#    emulates an injective fusion map (ψ_c). It maps unique combinations
#    of predecessor tags to unique output tags, preventing the arbitrary
#    collisions of a standard hash function and more closely modeling the
#    paper's information-preserving fusion mechanism.
# ==============================================================================

import numpy as np
import matplotlib.pyplot as plt
from collections import Counter
from tqdm import tqdm

class InjectiveFusionTable:
    """
    A class to emulate an injective fusion map (ψ_c).
    It maps unique tuples of predecessor tags to unique output tags.
    """
    def __init__(self, q):
        self.q = q
        self.mapping = {}
        self.next_tag = 0

    def fuse(self, pred_tags_tuple):
        """
        Takes a tuple of predecessor tags and returns a deterministic output tag.
        """
        # Ensure the tuple is canonical (sorted)
        pred_tags_tuple = tuple(sorted(pred_tags_tuple))
        
        if pred_tags_tuple not in self.mapping:
            # If this combination is new, assign it the next available tag
            self.mapping[pred_tags_tuple] = self.next_tag
            # Cycle through the available tags from 0 to q-1
            self.next_tag = (self.next_tag + 1) % self.q
        
        return self.mapping[pred_tags_tuple]

def build_layered_dag(num_layers, nodes_per_layer, R_max):
    """Builds a layered Directed Acyclic Graph (DAG) to serve as the "universe"."""
    nodes = {}
    for layer in range(num_layers):
        nodes[layer] = list(range(layer * nodes_per_layer, (layer + 1) * nodes_per_layer))

    predecessors = {node: [] for layer_nodes in nodes.values() for node in layer_nodes}
    
    # Define observer slice as all nodes NOT in the first layer
    first_layer_nodes = set(nodes.get(0, []))
    all_nodes = set(predecessors.keys())
    observer_slice_nodes = list(all_nodes - first_layer_nodes)

    for i in range(num_layers - 1):
        current_layer_nodes = nodes[i]
        next_layer_nodes = nodes[i+1]
        
        for node in current_layer_nodes:
            if R_max > 0 and len(next_layer_nodes) > 0:
                size = min(R_max, len(next_layer_nodes))
                connections = np.random.choice(next_layer_nodes, size=size, replace=False)
                for conn in connections:
                    predecessors[conn].append(node)
                
    return predecessors, list(first_layer_nodes), observer_slice_nodes

def shannon_entropy(tags):
    """Calculates the Shannon entropy of a list of tags."""
    if not tags:
        return 0
    n = len(tags)
    counts = Counter(tags)
    entropy = 0.0
    for count in counts.values():
        p_x = count / n
        if p_x > 0:
            entropy -= p_x * np.log2(p_x)
    return entropy

def run_simulation(q, R, num_layers, nodes_per_layer, ticks, hidden_layer_sequence):
    """
    Runs a single simulation for a given (q, R) pair.
    """
    # 1. Build the universe
    predecessors, first_layer_nodes, observer_slice_nodes = build_layered_dag(num_layers, nodes_per_layer, R)
    num_total_nodes = num_layers * nodes_per_layer

    # **AMENDMENT 4: Injective Fusion Rule**
    fusion_table = InjectiveFusionTable(q)
    
    tags = {node: 0 for node in range(num_total_nodes)}
    
    # **AMENDMENT 2: Entropy on Observer Slice**
    observed_tags = [tags[node] for node in observer_slice_nodes]
    entropy_history = [shannon_entropy(observed_tags)]
    
    sequence_idx = 0

    for _ in range(ticks):
        new_tags = {}
        for node in range(num_total_nodes):
            # **AMENDMENT 1: Deterministic Hidden Layer**
            # The first layer draws from the pre-computed sequence.
            if node in first_layer_nodes:
                new_tags[node] = hidden_layer_sequence[sequence_idx] % q
                sequence_idx += 1
                continue

            pred_list = predecessors[node]
            # Handle nodes with no predecessors in deeper layers (unlikely but possible)
            if not pred_list:
                 new_tags[node] = tags[node]
                 continue
            
            pred_tags = [tags[p] for p in pred_list]
            new_tags[node] = fusion_table.fuse(pred_tags)

        tags = new_tags
        
        # Calculate entropy only on the observer slice
        observed_tags = [tags[node] for node in observer_slice_nodes]
        entropy_history.append(shannon_entropy(observed_tags))

    entropy_increments = np.diff(entropy_history)
    stable_increments = entropy_increments[int(ticks * 0.2):]
    
    return np.mean(stable_increments) if len(stable_increments) > 0 else 0

def main():
    """Main function to run the parameter search and visualize the results."""
    print("Starting amended simulation to test the Parameter Elimination Theorem...")
    
    # --- Simulation Parameters ---
    # **AMENDMENT 3: Increased Scale**
    Q_RANGE = range(2, 16)  # Test alphabet sizes from 2 to 15
    R_RANGE = range(2, 16)  # Test connectivities from 2 to 15
    
    NUM_LAYERS = 10
    NODES_PER_LAYER = 50
    TICKS = 200

    # **AMENDMENT 1: Pre-compute the deterministic hidden sequence**
    # This sequence is the same for all (q, R) runs.
    hidden_sequence_size = NODES_PER_LAYER * TICKS
    print(f"Pre-computing a deterministic hidden sequence of size {hidden_sequence_size}...")
    # Use a fixed seed for reproducibility
    np.random.seed(42) 
    hidden_layer_sequence = np.random.randint(0, 1000, size=hidden_sequence_size)

    results = np.zeros((len(Q_RANGE), len(R_RANGE)))
    
    with tqdm(total=len(Q_RANGE) * len(R_RANGE), desc="Simulating (q, R) pairs") as pbar:
        for i, q in enumerate(Q_RANGE):
            for j, R in enumerate(R_RANGE):
                avg_entropy_increment = run_simulation(
                    q=q, R=R, num_layers=NUM_LAYERS,
                    nodes_per_layer=NODES_PER_LAYER, ticks=TICKS,
                    hidden_layer_sequence=hidden_layer_sequence
                )
                results[i, j] = avg_entropy_increment
                pbar.update(1)

    # --- Find and Print the Maximizer ---
    max_val = np.max(results)
    if max_val <= 0:
        print("\n--- Simulation Warning ---")
        print("The simulation did not generate significant entropy.")
        q_star, R_star = Q_RANGE[0], R_RANGE[0]
    else:
        max_indices = np.unravel_index(np.argmax(results, axis=None), results.shape)
        q_star = Q_RANGE[max_indices[0]]
        R_star = R_RANGE[max_indices[1]]
    
    print("\n--- Simulation Complete ---")
    print(f"The parameter pair that maximizes observer entropy generation is:")
    print(f"  q* = {q_star} (Alphabet Size)")
    print(f"  R* = {R_star} (Max Connectivity)")
    print(f"  Maximum Average Entropy Increment: {max_val:.6f} bits/tick")

    # --- Visualize the Results ---
    fig, ax = plt.subplots(figsize=(12, 10))
    cax = ax.imshow(results.T, interpolation='nearest', origin='lower',
                    cmap='viridis', aspect='auto')
    
    fig.colorbar(cax, label='Average Observer Entropy Increment per Tick')
    
    ax.set_title(f'Observer Entropy Generation vs. System Parameters (q, R)\nMaximizer at (q*, R*) = ({q_star}, {R_star})', fontsize=16)
    ax.set_xlabel('q (Alphabet Size)', fontsize=14)
    ax.set_ylabel('R (Max Connectivity)', fontsize=14)
    
    ax.set_xticks(np.arange(len(Q_RANGE)))
    ax.set_yticks(np.arange(len(R_RANGE)))
    ax.set_xticklabels(Q_RANGE)
    ax.set_yticklabels(R_RANGE)
    
    if max_val > 0:
        ax.scatter(max_indices[0], max_indices[1], color='red', s=200,
                   edgecolors='white', marker='*', label=f'Maximizer ({q_star}, {R_star})')
    ax.legend()
    
    plt.tight_layout()
    plt.show()

if __name__ == '__main__':
    main()
